...
% -----------------------------------------------------------------------------
\begin{frame}{Where we left off (Aug 12) --- and where each ask stands}
...
Context window into the action space & Wenqi & authority measured now (F4 running); as action, staged behind the decision-test verdict \\
...
% -----------------------------------------------------------------------------
\begin{frame}{What we built since Aug 12 (all reproducible, all pushed)}
\small
...
% -----------------------------------------------------------------------------
\begin{frame}{F1 --- the knob has authority (untrained, forced levels)}
\begin{columns}[T]
...
% -----------------------------------------------------------------------------
\begin{frame}{Training runs: the matrix so far}
\begin{center}\footnotesize
...
B2 & --- (control) & 0 & lr 3e-6, no KL & 60 & done, eval in progress \\
...
% -----------------------------------------------------------------------------
\begin{frame}{Finding 1 --- training cuts energy 32\,\% without losing success}
\begin{columns}[T]
\column{0.5\textwidth}
...
\begin{itemize}\itemsep 0.4em
  ...
  \item But: identical at $\lambda = 0$, $0.5$, $1.5$. \textbf{The energy term did not cause it}
        --- GiGPO itself does (format compliance, invalid-action penalty, shorter reasoning).
\end{itemize}
\end{columns}
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{Finding 2 --- the agent does not (yet) DECIDE: the choice collapses}
...
% -----------------------------------------------------------------------------
\begin{frame}{Why it collapses --- diagnosis with evidence, and the fix per cause}
\begin{center}\footnotesize
\begin{tabular}{@{}p{5.9cm}p{5.6cm}l@{}}
\toprule
...
\end{center}
\vspace{0.3em}\small
Plus the direct \textbf{decision test} (queued): evaluate A2@60 with its effort \emph{forced} to
low/mid/high vs its own free choice --- if free $\succ$ forced, the choices carry information.
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{Running right now (all unattended, monitored, resumable)}
\begin{center}\small
\begin{tabular}{@{}llp{6.2cm}@{}}
\toprule
job & what & decides \\
\midrule
eval-f3 & B2 series + decision test + budget profiling & does $\lambda$ separate once the policy learns? do its choices matter? does base react to a budget zero-shot? \\
...
F4 & context-knob authority (full/3/1 history) & the second knob's F1 \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.4em}\small
...
% -----------------------------------------------------------------------------
\begin{frame}{Measurement lessons that go straight into the paper}
\small
...
% -----------------------------------------------------------------------------
\begin{frame}{What we can claim today / what we are one result away from}
\small
...
\begin{itemize}\itemsep 0.2em
  ...
  \item A precise negative result with mechanism: success-gated multiplicative rewards at
        $\lambda \in \{0, 0.5, 1.5\}$ do \emph{not} make the policy exercise the knob.
\end{itemize}
\vspace{0.4em}
\textbf{One result away (this week):}
\begin{itemize}\itemsep 0.2em
  \item A2 vs B2 full series $\Rightarrow$ does $\lambda$ separate once the policy learns?
  \item Decision test $\Rightarrow$ do the trained policy's choices beat any fixed level?
...
% -----------------------------------------------------------------------------
\begin{frame}{Asks and decisions}
